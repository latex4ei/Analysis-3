...
\begin{sectionbox}
	...
	$f(t) = \frac{1}{2\pi} \int\limits_{-\infty}^\infty F(\omega) \exp(\i \omega t) \diff \omega = \begin{cases} f(t) &, f \text{ stetig in }t \\ \frac{f(t^-) + f(t^+)}{2} &, f\text{ unstetig }t \end{cases}$
\end{sectionbox}
...
\begin{sectionbox}
	...
	\begin{cookbox}{Lösen der pDGL mit dem Separationsansatz}
		\item Setze \(u(x,y)=f(x)g(y) \) in die pDGL ein und erhalte zwei gDGLen f\"ur \(f\) und \(g\). $u_{xx} = f''(x)g(x)$ und $u_{yy}=f(x)g''(x)$
